...
class SalesOrderDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final linkedInvoice = order.invoiceId.isNotEmpty
        ...
        : null;
    final orderHolds = context
        .watch<StockProvider>()
        .activeHolds
        .where(
          (hold) =>
              hold.sourceType == StockHoldSourceType.salesOrder &&
              hold.sourceId == order.id,
        )
        .toList();
    final heldQty = orderHolds.fold<int>(
      0,
                            ...
                            Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  Text(
                                    'Held Qty: $heldQty (${orderHolds.length} hold${orderHolds.length > 1 ? 's' : ''})',
                                    style: const TextStyle(
                                      color: AppTheme.warningColor,
                                      fontWeight: FontWeight.w600,
                                      fontSize: 13,
  ...
  Widget _buildActions(
    ...
    final canCancel =
        user?.hasPermission(AppPermissions.cancelSalesOrders) ?? false;
  ...
  Future<void> _confirmOrder(
    ...
    final success = await context.read<SalesOrderProvider>().updateOrder(
      updated,
  ...
  Future<void> _confirmDelete(
    ...
    if (!confirm || !context.mounted) return;
    final success = await context.read<SalesOrderProvider>().deleteOrder(
      order.id,
    );
